...
{
    // std::cout << "modcalled " << base << " " << exp << " " << mod << std::endl;
    int res = 1;
...
unsigned short calc_pi_digit(int start)
{
    // Count starting 3
    start -= 1;
    ...
    return (unsigned short)floor(res);
}

char hex_digit(unsigned short digit)
{
...
{
    for (unsigned int i = 0; i < std::min(a.length(), std::min(b.length(), c.length())); i++)
    {
        if (a[i] != b[i] || a[i] != c[i])
            return i;
    }
...
int main()
{
    ...
    {
        int digit = calc_pi_digit(i);
        i++;

        // Compute new pi
        new_pi += hex_digit(digit);
    ...
    // if (calc_pi.substr(0, new_pi.size()) == new_pi)
    //     std::cout << "OK!" << std::endl;
    // else
    //     std::cout << "ERROR!" << std::endl;
    std::cout << "Pi = " << new_pi << std::endl;
}